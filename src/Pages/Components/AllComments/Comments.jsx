...
const Comments = () => {
  ...
  const handleEdit = (e) => {
    ...
    navigate(`/editcomments/${e.currentTarget.id}`, {
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      await api.call(endpoints.postComment, editConfig);
  ...
  const handleRemove = async (e) => {
    ...
    try {
      await api.call(endpoints.removeComment, editConfig);
      window.location.reload();
    } catch (error) {}
  };
          ...
          {comments.length > 0 ? (
            <Box pb={4}>
              {comments.map((comment, i) => (
                <div className="parent-div" key={comment._id}>
                  <div className="parent-name">
                    <p>{comment.commenter.username}</p>
                    <p>Posted at: {comment.createdAt.split("T")[0]}</p>
                  </div>
                  ...
                  <div className="button-container">
                    ...
                    {decoded._id === comment.commenter._id && (
                      <button
                        className="button-reply"
                        onClick={handleEdit}
                        id={comment._id}
                    ...
                    {commentId === comment._id && (
                      <ReplyComment
                        commentId={comment._id}
                        poster={comment.commenter._id}
                      />
                    )}
                  </div>

                  {comment.children.map((elem, c) => (
                    <div className="children-div" key={elem._id}>
                      <div className="parent-name">
                        <p>{elem.commenter.username}</p>
                        <p>Posted at: {comment.createdAt.split("T")[0]}</p>
                      </div>
                      ...
                      <div className="button-container">
                        ...
                        {decoded && decoded._id === elem.commenter._id && (
                          <button
                            className="button-reply"
                            onClick={handleEdit}
                            id={elem._id}
                        ...
                        {(decoded._id === elem.commenter._id ||
                          auth.role === "ADMIN") && (
                          <button
                            className="button-remove"
                            onClick={handleShow2}
                        ...
                        {commentId === elem._id && (
                          <ReplyComment
                            commentId={elem._id}
                            poster={elem.commenter._id}
                          />
                        )}
                      </div>

                      {elem.children.map((child, k) => (
                        <div className="third-child" key={child._id}>
                          <div className="parent-name">
                            <p>{child.commenter.username}</p>
                            <p>Posted at: {child.createdAt.split("T")[0]}</p>
                          </div>
                          ...
                          <div className="button-container">
                            ...
                            {decoded && decoded._id === child.commenter._id && (
                              <button
                                className="button-reply"
                                onClick={handleEdit}
                                id={child._id}
                            ...
                            {decoded &&
                              (decoded._id === child.commenter._id ||
                                auth.role === "ADMIN") && (
                                <button
                                  className="button-remove"
                                  onClick={handleShow3}
                            ...
                            {commentId === child._id && (
                              <ReplyComment
                                commentId={child._id}
                                poster={child.commenter._id}
                              />
                            )}
                          </div>

                          {child.children.map((fourth, f) => (
                            <div className="fourth-child" key={fourth._id}>
                              <div className="parent-name">
                                {fourth.commenter.username}

                                <p>
                                  Posted at: {fourth.createdAt.split("T")[0]}
                                </p>
                              </div>
                              ...
                              <div className="button-container">
                                ...
                                {decoded &&
                                  decoded._id === fourth.commenter._id && (
                                    <button
                                      className="button-reply"
                                      onClick={handleEdit}
                                      id={fourth._id}
                                ...
                                {commentId === fourth._id && (
                                  <ReplyComment
                                    commentId={fourth._id}
                                    poster={fourth.commenter._id}
                                  />
                                )}
                              </div>
                              {fourth.children.map((fifth, five) => (
                                <div className="fifth-child" key={five._id}>
                                  <div className="parent-name">
                                    {fifth.commenter.username}

                                    <p>
                                      Posted at: {fifth.createdAt.split("T")[0]}
                                    </p>
                                  </div>
                                  ...
                                  <div className="button-container">
                                    ...
                                    {decoded &&
                                      decoded._id === fifth.commenter._id && (
                                        <button
                                          className="button-reply"
                                          onClick={handleEdit}
                                          id={fifth._id}
